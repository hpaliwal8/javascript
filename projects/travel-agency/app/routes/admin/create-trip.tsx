...
export const loader = async () => {
    const response = await fetch('https://restcountries.com/v3.1/all?fields=name,flag,latlng,coords,maps');
    const data = await response.json();
    return data.map((country: any) => ({
        name: country.flag + country.name.common,
        coordinates: country.latlang,
...
const createTrip = ({ loaderData }: Route.ComponentProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (formData.duration <= 0 || formData.duration > 15) {
            setError("Duration should be between 1 and 15 days");
            setLoading(false);
            return;
        }
